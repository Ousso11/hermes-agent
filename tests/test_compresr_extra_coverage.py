...
def test_store_original_no_env_unknown_backend_fails_open(monkeypatch, tmp_path):
    ...
    assert path is None
    # the host file must be cleaned up when visibility can't be proven
    assert not (hermes_home / "cache" / "compresr" / "tool-output" / "dt").exists()
...
def test_prune_total_under_budget_is_noop(tmp_path):
    ...
    _prune_cache_dir_via_backend(None, str(tmp_path), 1000, str(tmp_path / "keep"))
